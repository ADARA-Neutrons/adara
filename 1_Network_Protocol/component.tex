...
Additionally, we'd like to use a bit to indicate leading/trailing edge of
detection. Leading edge would be indicated by a zero, and would be the
only value initially sent.}
...
\begin{figure}[h]
  ...
  \begin{bytefield}{32}
    \bitheader{31,0} \\
    \wordbox{1}{Length 20 + Section Lengths} \\
...
\begin{itemize}
\item{\bf Length} is defined by the number of events in the packet. There
are 20 bytes of pulse and packet information followed by 8 bytes of data
for each event.
\item{\bf Pulse ID} identifies the most recent pulse generated by the
accelerator. This ID may not be orginating pulse for the events in this packet
due to frame skew.
...
\begin{figure}[h]
  ...
  \begin{bytefield}{32}
    \bitheader{31,30,0} \\
    \bitbox{1}{\tiny E \\ D \\ G} &
    \bitbox{30}{Time-of-flight (units of 100 ns)}
  \end{bytefield}
...
\begin{itemize}
\item{\bf Length} is defined by the number of events in the packet. There
are 20 bytes of pulse and packet information followed by 4 bytes of data
for each event.
\item{\bf Pulse ID} identifies the most recent pulse generated by the
...
of 100 nanoseconds of the detected neutrons. If this is from the trailing edge
of the discriminator, bit {\bf EDG} will be set.
\end{itemize}
...
name for the beamline that generated this data. These strings are concatenated
together without intervening NULLs. This field must be padded to a multiple of
4 bytes.
\end{itemize}
