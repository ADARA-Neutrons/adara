...
allow parsers to skip over packets that are unknown or irrelevant to the task
at hand.  For example, the SMS will use the same format for stable storage
as it uses for network traffic. The SMS will place synchronization packets
(section~\ref{section:protocol_sync}) onto stable storage which may be
visible to live-event clients due to the mechanism used for transmission.
Those clients would ignore the synchronization packets as irrelevant to
their funciton.
