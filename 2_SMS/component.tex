...
failure. Bit 31 is currently used in the raw event packets to denote an
error. It has not been clarified if the rest of the ID and TOF data carry
actual data or is garbage. Assuming valid data in the error events, the
SMS can put those into a separate packet type for errors, freeing up bit
31 in the processed packets for mapping errors.}
...
\todo{Statistics Reset}{%
The SMS will allow the programmatic insertion of ``statistics reset'' packets
into the aggregated stream via the rising edge of an EPICS PV it exports.
Need to find a place to put that in the document.}
