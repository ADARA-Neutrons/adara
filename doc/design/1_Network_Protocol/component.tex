...
\begin{table}
  \begin{center}
	...
	0x4102.0 & Pixel Mapping Alternate Table &
		\ref{section:protocol_pixel_mapping_alt_table} \\
	0x4003.0 & Run Status &
	...
	0x400D.0 & Beam Monitor Config &
		\ref{section:protocol_beam_monitor_config} \\
...
in Section~\ref{section:protocol_pixel_mapping_table}
or \ref{section:protocol_pixel_mapping_alt_table}).
...
\item Pixel Mapping Alt Table (Section~\ref{section:protocol_pixel_mapping_alt_table})
...
\item Beam Monitor Config (Optional, Section~\ref{section:protocol_beam_monitor_config})
...
analysis. The Stream Management Service (SMS) will transform the pixel IDs
...
(Section~\ref{section:protocol_rtdl_packet})
and Raw Event Data (Section~\ref{section:protocol_raw_event_packet})
packet types.
...
\newpage
\subsubsection{Pixel Mapping Table Packet (to be deprecated...)}
...
High Flux Isotope Reactor (HFIR) ``WAND'' Beamline (HB2C),
...
described here will ultimately be phased out in favor of the
more flexible ``Pixel Mapping Table Alternate'' packet type
(see Section~\ref{section:protocol_pixel_mapping_alt_table}).

The ``Pixel Mapping Table'' packet defined in
...
the {\bf Base Logical ID}. For example, the pixel in {\bf Physical ID 1} maps
to logical pixel {\bf Base Logical ID}. {\bf Physical ID 2} maps to logical
pixel ${\bf Base Logical ID} + 1$, and so on.  The bank ID for each mapping is
given by the section header.

Note that a {\it Single Bank} may be described by {\bf multiple sections}
if the logical ID space is not contiguous,
{\bf OR} if the total number of pixel IDs in the bank is ``too large,''
i.e. specifically more than 65535
(the capacity of the 16-bit section {\bf Count}).


\newpage
\subsubsection{Pixel Mapping Table Alternate Packet}
\label{section:protocol_pixel_mapping_alt_table}
...
\begin{figure}[h]
  ...
  \end{bytefield}
  \caption{Packet Type 0x4102.0: Pixel Map Alt Packet}
  \label{fig:protocol_packet_pixel_map_alt}
\end{figure}
...
High Flux Isotope Reactor (HFIR) ``WAND'' Beamline (HB2C),
there exists the potential for {\it Non One-to-One} pixel mappings.
Therefore, this new ``Alternate'' Pixel Mapping Table packet type
...
(Note that the original ``Pixel Mapping Table'' packet
...
packet type.)
The ``Pixel Mapping Table Alternate'' packet type is effectively an
...
The ``Pixel Mapping Table Alternate'' packet defined in
Figure~\ref{fig:protocol_packet_pixel_map_alt} will preserve any table for
consumers of the event stream. This packet must be present near the beginning
of a new experiment recording, but may occur multiple times in the transmitted
stream. Each additional copy transmitted must be identical to the first copy
for the same experiment recording.
...
\begin{figure}[ht]
  ...
  \end{bytefield}
  \caption{Pixel Map Alt Section}
  \label{fig:protocol_packet_pixel_map_alt_section}
\end{figure}
...
To reduce the size of the mapping table, it is described using sections as
defined in Figure~\ref{fig:protocol_packet_pixel_map_alt_section}.
...
Each entry in the section describes a physical to logical (and bank) mapping.
Each entry gives the logical ID for the mapping, and the physical ID is
determined by adding the entry's zero-based index (from the section header) to
the {\bf Base Physical ID}. For example, the pixel in {\bf Logical ID 1} maps
to physical pixel {\bf Base Physical ID}. {\bf Logical ID 2} maps to physical
pixel ${\bf Base Physical ID} + 1$, and so on.  The bank ID for each mapping is
given by the section header.
...
\begin{itemize}
\item{\bf Length} of the payload is now 8 bytes,
...
(Real Time Data Link, Section~\ref{section:protocol_rtdl_packet})
packet bits.
\item{\bf Id Len} gives the length of the numeric beamline identifier in the
{\bf Beamline Data} field.
\item{\bf Short Len} gives the length of the short name for the beamline in the
...
\newpage
\subsubsection{Beam Monitor Config}
\label{section:protocol_beam_monitor_config}

\begin{figure}[h]
  \centering
  \begin{bytefield}[bitwidth=1em]{32}
    \bitheader{31,0} \\
    \wordbox{1}{Length $24 N + 4$ (N Sections, 24 bytes each)} \\
  ...
  \end{bytefield}
  \caption{Packet Type 0x400D.0: Beam Monitor Config}
  \label{fig:protocol_packet_monitor_config}
\end{figure}
...
The ``Beam Monitor Config'' packet described in
Figure~\ref{fig:protocol_packet_monitor_config} is generated by the
...
\begin{itemize}
\item{\bf Length} is 4 bytes for the Beam Monitor Count
...
\begin{figure}[h]
  ...
  \begin{bytefield}[bitwidth=1em]{32}
    \bitheader{31,0} \\
    \wordbox{1}{Length $((15 + M)4N) + 4$ (N Sections, 60 + 4M bytes each)} \\
...
\begin{itemize}
\item{\bf Length} of payload is 4 bytes for the Detector Bank Set Count,
...
In order for the Streaming Translation Service (STS) client to
...
of the socket connection between the SMS and STS)
the SMS can send a ``Data Done'' packet to explicitly inform the STS
...
while the STS finishes its processing of the stream into a NeXus data file,
...
(greater than 1 minute) by the STS,
{\it after} the last data has been streamed from the SMS,
can result in an apparent {\it total loss} of SMS-STS network connectivity.
...
something about the fact that the STS service runs through a firewall,
...
e.g. with the SMS and STS together on a local workstation,
so the issue is not likely related to XInetd service management.
Indeed, the ``keepalive'' setting has been activated for the STS service
...
\begin{itemize}
\item{\bf Length} is 16 bytes plus the length
