...
for analysis.
So normally the Stream Management Service must transform the pixel ID
...
\item Beam Monitor Config (Section~\ref{section:protocol_beam_monitor_config})
...
analysis. The Stream Management Service will transform the pixel IDs from
...
\begin{figure}[h]
  ...
  \begin{bytefield}{32}
    ...
    \bitheader{31,0} \\
    \wordbox{1}{Beam Monitor Count (u32)} \\
...
Stream Management Service and is used to convey optionally requested
...
any TOF less than this will simply be discarded during histogram
accumulation.
...
values greater than this will be included in the final TOF bin for the
given histogram.
\item{\bf Histogram Bin Size} is the linear ``bin size'' (in microseconds)
for histogramming the beam monitor's TOF data values,
e.g. the number of TOF values to be summed together in a single
histogram bin.
