...
\begin{figure}[h]
  ...
  \end{bytefield}
  \caption{Packet Type 0x4002.0: Pixel Map Packet (Deprecated...)}
...
The ``Pixel Mapping Table'' (Deprecated...) packet defined in
...
\begin{figure}[ht]
  ...
  \end{bytefield}
  \caption{Pixel Map Section (Deprecated...)}
...
\begin{figure}[h]
  ...
  \end{bytefield}
  \caption{Packet Type 0x4102.0: Pixel Map Alt Packet V0}
...
``Pixel Mapping Table'' (Deprecated...) packet type,
...
Figure~\ref{fig:protocol_packet_pixel_map_alt_v0} will preserve any table for
consumers of the event stream. This packet must be present near the beginning
of a new experiment recording, but may occur multiple times in the transmitted
stream. Each additional copy transmitted must be identical to the first copy
for the same experiment recording.
...
\begin{figure}[ht]
  ...
  \end{bytefield}
  \caption{Pixel Map Alt Section - Version 0}
...
\begin{figure}[h]
  ...
  \begin{bytefield}[bitwidth=1em]{32}
    ...
    \wordbox{2}{Mapping Data ($N - 4$ bytes)} \\
  \end{bytefield}
  \caption{Packet Type 0x4102.1: Pixel Map Alt Packet V1}
...
and indeed can and should be subsequently used
for any or all pixel mapping tables from this point forward.
(Note that the original ``Pixel Mapping Table'' (Deprecated...) packet
described in Section~\ref{section:protocol_pixel_mapping_table}
will ultimately be phased out in favor of this new ``Alternate''
packet type.)
...
storage optimization ``Shorthand'' option,
to greatly reduce the size of the packet in transit
for ``Very Large'' Pixel Mapping Tables
(e.g. for the Timepix3 Neutron Camera at the HFIR IMAGING/CG1D beamline,
which produces $4096 x 4096$ or over 16 million pixel IDs,
for a very large resulting packet size of over 60 megabytes...!).

The ``Pixel Mapping Table Alternate'' V1 packet defined in
Figure~\ref{fig:protocol_packet_pixel_map_alt_v1} will preserve any table for
consumers of the event stream. This packet must be present near the beginning
of a new experiment recording, but may occur multiple times in the transmitted
stream. Each additional copy transmitted must be identical to the first copy
for the same experiment recording.
...
\begin{figure}[ht]
  ...
  \begin{bytefield}[bitwidth=1em]{32}
    ...
    \wordbox{1}{Logical ID $P$ (u32)} \\
  \end{bytefield}
  \caption{Pixel Map Alt Section - Version 1, ``Direct'' Format}
...
\begin{figure}[ht]
  ...
  \begin{bytefield}[bitwidth=1em]{32}
    ...
    \bitbox{16}{Physical Step (``$STEP_{Physical}$'', u16)} &
    \bitbox{16}{Logical Step (``$STEP_{Logical}$'', u16)} \\
    \bitbox{32}{Stopping Logical ID (``$STOP_{Logical}$'', u32)} \\
  \end{bytefield}
  \caption{Pixel Map Alt Section - Version 1, ``Shorthand'' Format}
...
between the clusters.
To reduce the size of the mapping table, it is described using sections as
defined in Figure~\ref{fig:protocol_packet_pixel_map_alt_section_v1_direct}
or \ref{fig:protocol_packet_pixel_map_alt_section_v1_shorthand}.
...
The now-15-Bit {\bf Count} field gives the number of
mapping entries in this section, and {\bf Bank ID} describes the
detector bank containing these pixels.

Here, each entry in the section describes a physical to logical
(and the bank) mapping.
Each entry gives the logical ID for the mapping, and the physical ID is
determined by adding the entry's zero-based index (from the section header)
to the {\bf Base Physical ID}.
For example, the pixel in {\bf Logical ID 1} maps
to physical pixel {\bf Base Physical ID}.
{\bf Logical ID 2} maps to physical pixel ${\bf Base Physical ID} + 1$,
and so on.
The bank ID for each mapping is given by the section header.
...
Here, everything is completely different. Lol... ;-D

Note that in either formatting case here,
a {\it Single Bank} may be described by {\bf multiple sections}
if the physical/logical ID space is not contiguous,
{\bf OR} if the total number of pixel IDs in the bank is ``too large,''
i.e. specifically more than {\it 32767},
which is the capacity of the now-15-Bit section {\bf Count}.
(Note the reduction of 1 Bit in the {\bf Count} field
for this Pixel Map Alt V1 Packet,
to be able to distinguish between the ``Direct''
