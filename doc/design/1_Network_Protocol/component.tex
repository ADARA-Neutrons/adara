...
\begin{table}
  \begin{center}
	...
	0x0003.0 & Mapped Event Data &
		\ref{section:protocol_mapped_event_packet} \\
...
\item{\bf Veto} contains the veto information
...
\begin{table}[h]
  \begin{center}
	...
	0x000002 & VETO\_NOT\_TARGET\_1 & Beam was not for target 1 \\
	0x000004 & VETO\_NOT\_TARGET\_2 & Beam was not for target 2 \\
...
\newpage
\subsubsection{Detector System: Mapped event data}
\label{section:protocol_mapped_event_packet}
...
\begin{figure}[h]
  ...
  \begin{bytefield}{32}
    ...
    \wordbox{2}{Event $N$} \\
  \end{bytefield}
  \caption{Packet Type 0x3.0: Mapped Event Data}
  \label{fig:protocol_packet_mapped_event}
\end{figure}

The ``Mapped Event Data'' packet described in
Figure~\ref{fig:protocol_packet_mapped_event} is generated by the neutron
detector system and is used to convey time-of-flight and physical location
information for detected neutrons,
{\it almost exactly} like the ``Raw Event Data'' packet described
in Section \ref{section:protocol_raw_event_packet},
...
As discussed in Section \ref{section:protocol_banked_events_v1}
...
and is done using a one-to-one ``Pixel Mapping Table''
(communicated in the stream via packets defined
...
(Sections~\ref{section:protocol_banked_events_v1} and
\ref{section:protocol_beam_monitor_events}).
...
(see Table~\ref{table:sns_timing_master_veto_bits}),
e.g. as are included in the RTDL
...
After first connecting to the Stream Management Service (SMS), a client will send
...
namely the ``Target Number'' (see below).
...
by ensuring that the Target Number simply defaults to ``1''
...
startup time of the SMS, or the time the packet was send, if not another value
altogether.
\item{\bf Target Number} specifies the Neutron Target Station Number
for the given Neutron Facility,
