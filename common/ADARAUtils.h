...
/*! \class StatWindow
 *
 * The StatWindow class efficiently calculates the average count value,
 * along with the corresponding sample standard deviation (a la Bessel),
 * to deduce a projected two standard deviation (95%) sample inclusion
 * count from a sliding statistical window over a sequence of count values.
 */
class StatWindow
{
public:
    /// Default constructor.
    StatWindow(uint32_t start_value, uint32_t window_size)
		: m_window_size(window_size), m_mean_sum(0), m_var_sum(0),
		m_bessel_count(1), m_stddev(0), m_count(0), m_mean(0),
		m_reserve(1), m_update_count(0), m_full(false)
    {
		// Make Sure Window Size is Tractable... ;-D
		if ( m_window_size < 1 )
			m_window_size = 1;

		// Allocate Count Vector Storage...
		for ( uint32_t i=0 ; i < m_window_size ; i++ )
		{
			m_count_data.push_back(0);
			m_dev_data.push_back(0);
		}

		// Prime the Pump with 1st Value...
		nextValue( start_value );
	}

	/// Add New Value to Sequence and Return Latest Window Average
	uint32_t nextValue
    (
        uint32_t a_value  ///< Value to add to sequence
    )
	{
		// Window is Full, Subtract Former Values from Sums...
		if ( m_full ) {
			m_mean_sum -= m_count_data[ m_count ];
			m_var_sum -= m_dev_data[ m_count ];
		}

		// Record/Add Next Value in Count Sequence...
		m_count_data[ m_count ] = a_value;
		m_mean_sum += a_value;

		// Increment Index/Value Count (Set "Full" Flag)
		...
		// Update Mean...
		m_mean = ( m_full ) ?
			( m_mean_sum / m_window_size ) : ( m_mean_sum / m_count );

		m_reserve = 3 * m_mean / 2;
/*
		// Update Deviation Data and Variance Sum...
		int32_t dev2 = ( a_value - m_mean );
		dev2 *= dev2;
		m_dev_data[ old_count ] = dev2;  // m_count already incremented...
		m_var_sum += dev2;

		// Don't Update Reserve on Every Value Update... Take It Easy...
		// if ( !( m_update_count++ % 100 ) )
		{
			// Compute Standard Deviation...
			m_bessel_count = ( m_full ) ?
				( m_window_size - 1 ) : ( m_count - 1 );
			if ( m_bessel_count < 1 )
				m_bessel_count = 1;
			m_stddev = (uint32_t)
				sqrt( (double) m_var_sum / (double) m_bessel_count );

			// Return Suggested Reserve Size...
			m_reserve = m_mean + m_stddev + m_stddev;

			// Always Need to Reserve At Least 1 Slot,
			// Or We Wouldn't Be Here... ;-D
			m_reserve = ( m_reserve ) ? m_reserve : 1;
		}
*/
		return( m_reserve );
	}
...
//private:
	std::vector<uint32_t> m_count_data;
	std::vector<uint32_t> m_dev_data;
	uint32_t m_window_size;
	uint32_t m_mean_sum;
	uint32_t m_var_sum;
	uint32_t m_bessel_count;
	uint32_t m_stddev;
	uint32_t m_count;
	uint32_t m_mean;
	uint32_t m_reserve;
